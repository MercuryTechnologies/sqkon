...
open class KeyValueStorage<T : Any>(
     ...
     * ```
     *
     * Note: like [selectByKeys], this re-emits on every committed write to the store (one emission
     * per transaction), even when the resulting list is unchanged — it does **not**
     * `distinctUntilChanged`. [selectResult] and [metadata] do dedupe consecutive identical
     * emissions; add your own `.distinctUntilChanged()` if you want that here.
     */
    fun select(
        where: Where<T>? = null,
        orderBy: List<OrderBy<T>> = emptyList(),
    ...
    /**
     * Create a [PagingSource] that pages through results according to queries generated from the
     ...
     * infinite-scroll lists prefer [selectKeysetPagingSource], which avoids OFFSET's per-page
     * row-skipping (see its docs for the per-page scan cost).
     *
